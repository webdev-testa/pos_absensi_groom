    ...
    <AdminLayout>
      <div className="space-y-6 pb-12 font-sans text-foreground max-w-4xl mx-auto">
        ...
        {step === 1 && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-base font-bold font-heading text-foreground">
                  Langkah 1: Identitas Pemilik Kucing
                </h2>
                <p className="text-xs text-muted-foreground">
                  Cari pelanggan lama via nomor WhatsApp atau buat data owner baru.
                </p>
              </div>
            </div>
            ...
            <div className="relative">
              ...
              <Input
                placeholder="Ketik nomor WhatsApp atau nama owner (cth: 08123... / Fara)..."
                value={searchOwnerQuery}
                onChange={e => setSearchOwnerQuery(e.target.value)}
                className="pl-10 text-xs h-11 rounded-xl border-border bg-card"
              />
            </div>

            {/* Search Results */}
            {searchOwnerQuery.length >= 2 && (
              <div className="space-y-2">
                <div className="text-[11px] font-mono uppercase text-muted-foreground">Hasil Pencarian:</div>
                {isSearchingOwners ? (
                  <div className="text-xs text-muted-foreground py-2 font-mono">Mencari...</div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    {ownerSearchResults.map(owner => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-full bg-surface-soft flex items-center justify-center font-bold text-xs text-foreground">
                            {owner.nama.charAt(0)}
                          </div>
                          <div>
                            <div className="text-xs font-semibold text-foreground">{owner.nama}</div>
                            <div className="text-[11px] font-mono text-muted-foreground">{owner.no_wa}</div>
                          </div>
                        </div>
                        {selectedOwner?.id === owner.id && (
                  ...
                  <div className="p-4 bg-surface-soft border border-border rounded-xl text-xs text-muted-foreground text-center">
                    Tidak ditemukan pemilik dengan kata kunci tersebut.
                  </div>
                )}
              </div>
            )}

            {/* Switch to New Owner or Selected Summary */}
            <div className="pt-2">
              {!isNewOwner && !selectedOwner && (
                <Button
                  type="button"
                  variant="outline"
                  onClick={handleChooseNewOwner}
              ...
              {isNewOwner && (
                <Card className="bg-card border-border rounded-2xl p-4 space-y-3">
                  <div className="flex items-center justify-between border-b border-border/60 pb-2">
                    <span className="text-xs font-bold font-heading text-foreground">Form Pemilik Baru</span>
                    <Button
                      variant="ghost"
                      size="sm"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="space-y-1">
                      ...
                      <Input
                        placeholder="Cth: Kak Fara"
                        value={newOwnerData.nama}
                        onChange={e => setNewOwnerData(p => ({ ...p, nama: e.target.value }))}
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        onChange={e => setNewOwnerData(p => ({ ...p, no_wa: e.target.value }))}
                        className="text-xs h-9 rounded-xl border-border bg-background"
                      />
                    </div>
                    <div className="space-y-1 sm:col-span-2">
                      <label className="text-xs font-medium text-foreground">Alamat (Opsional)</label>
                      <Input
                        placeholder="Cth: Jl. Melati No. 12"
                        value={newOwnerData.alamat}
                        onChange={e => setNewOwnerData(p => ({ ...p, alamat: e.target.value }))}
                        className="text-xs h-9 rounded-xl border-border bg-background"
                      />
                    </div>
                  </div>
              ...
              <Button
                ...
                onClick={() => {
                  if (!selectedOwner && (!isNewOwner || !newOwnerData.nama || !newOwnerData.no_wa)) {
                    toast.error('Pilih owner atau isi form owner baru terlebih dahulu!')
                    return
                  }
                  setStep(2)
                }}
        ...
        <Dialog open={isSuccessModalOpen} onOpenChange={open => !open && setIsSuccessModalOpen(false)}>
          <DialogContent className="sm:max-w-lg bg-card border-border rounded-2xl p-6">
            ...
            {createdSession && (
              <div className="space-y-3 py-2 text-xs">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-2 p-2.5 bg-surface-soft border border-border rounded-xl">
                    ...
                    <Button
                      ...
                      className="h-7 px-2 text-xs text-muted-foreground hover:text-foreground cursor-pointer shrink-0"
                    >
                ...
                <div className="space-y-1">
                  ...
                  <textarea
                    ...
                    rows={7}
                    className="w-full text-xs font-mono text-foreground bg-surface-soft border border-border rounded-xl p-2.5 leading-relaxed resize-none focus:outline-none"
                  />
                </div>
              </div>
            )}
            ...
            <DialogFooter className="flex flex-col sm:flex-row gap-2 pt-2 border-t border-border/60">
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center justify-center text-xs h-9 px-3 rounded-xl border border-border text-foreground hover:bg-surface-soft cursor-pointer"
              >
