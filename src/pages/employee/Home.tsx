  ...
  const handleClockIn = async () => {
    if (hasClockedIn) {
      toast.info("Already clocked in", {
        description: "You have already recorded your attendance today.",
      });
      return;
    }
    ...
    try {
      ...
      toast.error("Clock In Failed", {
        description:
          error.message || "Make sure you allowed camera and location access.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleClockOut = async () => {
    try {
      ...
      toast.success("Clocked out", {
        description: "Attendance record reset for testing.",
      });
    } catch (error: any) {
  ...
  return (
    <div className="relative min-h-screen bg-[#F5F2ED] text-[#1A1814] overflow-hidden font-sans selection:bg-red-600/30">
      ...
      <header className="relative z-10 p-6 flex justify-between items-center bg-white border-b border-[#E0DDD7] shadow-sm">
        <div>
          <p className="text-[11px] font-medium text-[#A8A49E] tracking-[1.5px] uppercase font-mono mb-1">
            Welcome back
          </p>
          <h1 className="text-[20px] font-bold text-[#1A1814] font-['Syne'] capitalize">
            {userName}
          </h1>
        </div>
        <button
      ...
      <main className="relative z-10 flex flex-col items-center justify-center pt-16 pb-24 px-6 min-h-[calc(100vh-80px)]">
        ...
        <div className="text-center mb-16 space-y-2">
          ...
          <p className="text-[15px] text-[#6B6760] font-medium">{dateString}</p>
        </div>
          ...
          >
            {loading ? (
              <div className="flex flex-col items-center gap-3">
                ...
                <span className="font-semibold text-white tracking-wide text-[13.5px]">
                  Memproses...
                </span>
              </div>
            ) : hasClockedIn ? (
              <div className="flex flex-col items-center gap-3">
                <CheckCircle2
                  ...
                  strokeWidth={2.5}
                />
                <span className="font-['Syne'] font-bold text-[18px] tracking-wide">
                  Absen Berhasil
                </span>
              </div>
            ) : (
              <div className="flex flex-col items-center gap-2 text-white">
                ...
                <span className="text-[22px] font-['Syne'] font-bold tracking-wide">
                  CLOCK IN
                </span>
        ...
        <div className="mt-16 w-full max-w-sm">
          <div className="bg-white border border-[#E0DDD7] p-5 rounded-[16px] shadow-sm flex items-start gap-4">
            <div
              className={`p-3 rounded-full flex-shrink-0 ${hasClockedIn ? "bg-[#E2F0E8] text-[#2A7A4B]" : "bg-[#F5F2ED] text-[#6B6760]"}`}
            >
        ...
        {/* Dev Testing: Clock Out Button */}
        {hasClockedIn && (
          <div className="mt-4 w-full max-w-sm">
            <button
              onClick={handleClockOut}
              disabled={loading}
              className="w-full py-3 rounded-[16px] border border-[#E0DDD7] bg-white hover:bg-[#F5F2ED] text-[#6B6760] hover:text-[#C84B2F] transition-all text-[14px] font-medium flex items-center justify-center gap-2"
            >
              {loading ? (
