...
import { MapPin, CheckCircle2, ChevronRight, Camera, RefreshCw, Bell } from "lucide-react";
...
export default function EmployeeHome() {
  ...
  const dateStringFull = currentTime.toLocaleDateString('id-ID', { weekday: "long", day: "numeric", month: "short", year: "numeric" });
    ...
    return (
      <div className="min-h-screen bg-[#F0FAFF] flex flex-col font-sans">
        <div className="p-6 flex items-center gap-4 bg-white border-b border-[#C8E8F5] shadow-sm">
          <button onClick={() => setFlowState('idle')} className="w-10 h-10 rounded-xl bg-[#F0FAFF] border border-[#C8E8F5] flex items-center justify-center text-[#4A7A8A]">
            <ChevronRight className="rotate-180" size={20} />
          </button>
          <h1 className="font-['Syne'] text-[20px] font-bold text-[#1A3A4A]">Konfirmasi Absen</h1>
        </div>

        <div className="flex-1 p-6 overflow-y-auto">
          <div className="rounded-[20px] overflow-hidden border border-[#C8E8F5] relative bg-[#0D2D3D] shadow-sm h-[300px]">
            {photoUrl && <img src={photoUrl} alt="Selfie" className="w-full h-full object-cover" />}
            <div className="absolute bottom-0 left-0 right-0 bg-black/60 p-4 backdrop-blur-md">
              <div className="flex justify-between items-center">
                <div className="text-white/80 font-mono text-[11px] leading-relaxed">
                  <strong className="text-white block text-[13px] mb-1">{userName}</strong>
                  {timeString} · {dateStringFull}<br/>
          ...
          <div className="mt-4 bg-white border border-[#C8E8F5] rounded-[18px] p-4 flex items-center gap-3 shadow-sm">
            ...
            <div className="flex-1 min-w-0">
              <div className="font-medium text-[#1A3A4A] text-[14px] truncate">Lokasi Terdeteksi</div>
              <div className="font-mono text-[11px] text-[#4A7A8A] mt-0.5">{coords?.latitude.toFixed(5)}, {coords?.longitude.toFixed(5)}</div>
            </div>
          ...
          <div className="mt-4 flex gap-3">
            <div className="flex-1 bg-white border border-[#C8E8F5] rounded-[14px] p-3 shadow-sm">
              <div className="text-[10px] text-[#4A7A8A] font-mono uppercase tracking-wide mb-1">Jam Masuk</div>
              <div className="font-mono text-[16px] font-medium text-[#F5A940]">{timeString}</div>
            </div>
            <div className="flex-1 bg-white border border-[#C8E8F5] rounded-[14px] p-3 shadow-sm">
              <div className="text-[10px] text-[#4A7A8A] font-mono uppercase tracking-wide mb-1">Status Lokasi</div>
              <div className="font-medium text-[13px] text-[#3AAD7A]">Dalam Area</div>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 bg-white border-t border-[#C8E8F5]">
          ...
          >
            {loading ? <RefreshCw className="animate-spin" size={20} /> : "Konfirmasi Absen Masuk"}
          </button>
          <button 
  ...
  if (flowState === 'success') {
    return (
      <div className="min-h-screen bg-[#F0FAFF] flex flex-col items-center justify-center p-6 text-center font-sans">
        ...
        <h2 className="font-['Syne'] text-[28px] font-bold text-[#1A3A4A] mb-2">Absen berhasil!</h2>
        <p className="text-[14px] text-[#4A7A8A] leading-relaxed mb-8">
          Kehadiran kamu sudah tercatat.<br/>Selamat bekerja, {userName} 👋
        </p>
        ...
        <div className="w-full bg-white border border-[#C8E8F5] rounded-[20px] p-5 mb-8 shadow-sm text-left">
          <div className="flex justify-between items-center py-2 border-b border-[#F0FAFF]">
            ...
            <span className="font-mono text-[13.5px] font-medium text-[#1A3A4A]">{userName}</span>
          </div>
          <div className="flex justify-between items-center py-2 border-b border-[#F0FAFF]">
            <span className="text-[13px] text-[#4A7A8A]">Jam masuk</span>
            <span className="font-mono text-[13.5px] font-medium text-[#3AAD7A]">{timeString}</span>
          </div>
          <div className="flex justify-between items-center py-2 border-b border-[#F0FAFF]">
            <span className="text-[13px] text-[#4A7A8A]">Lokasi</span>
            <span className="font-mono text-[13.5px] font-medium text-[#3AAD7A]">Dalam area ✓</span>
          </div>
          <div className="flex justify-between items-center py-2">
            <span className="text-[13px] text-[#4A7A8A]">Koordinat</span>
            <span className="font-mono text-[11px] text-[#1A3A4A]">{coords?.latitude.toFixed(5)}, {coords?.longitude.toFixed(5)}</span>
          </div>
        </div>
  ...
  return (
    <div className="bg-[#F0FAFF] flex flex-col font-sans relative min-h-screen">
      ...
      <header className="relative z-10 p-6 flex justify-between items-center bg-white border-b border-[#C8E8F5] shadow-sm">
        ...
        <button
          className="w-[42px] h-[42px] rounded-full bg-white hover:bg-[#F0FAFF] text-[#4A7A8A] hover:text-[#F5A940] transition-all border border-[#C8E8F5] flex items-center justify-center relative shadow-sm"
        >
          <Bell size={18} />
      ...
      <div className="p-6">
        ...
        <div className="bg-white border border-[#C8E8F5] rounded-[24px] p-6 shadow-sm relative overflow-hidden mb-5">
          ...
          <div className="text-[13px] text-[#4A7A8A] mb-6">Jam kerja dimulai 08:00</div>

          <div className="flex gap-2.5 mb-5">
            <div className="flex-1 bg-[#F0FAFF] border border-[#C8E8F5] rounded-[14px] p-3">
              <div className="text-[10px] text-[#8ABAC8] uppercase tracking-wide font-mono mb-1">Status hari ini</div>
              <div className={`font-mono text-[14px] font-medium ${hasClockedIn ? 'text-[#3AAD7A]' : 'text-[#F5A940]'}`}>
                {hasClockedIn ? 'Sudah Absen' : 'Belum Absen'}
              </div>
            </div>
            <div className="flex-1 bg-[#F0FAFF] border border-[#C8E8F5] rounded-[14px] p-3">
              <div className="text-[10px] text-[#8ABAC8] uppercase tracking-wide font-mono mb-1">Kehadiran</div>
              <div className="font-mono text-[14px] font-medium text-[#4DC8F5]">26 Hari</div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-5">
          <div className="bg-white border border-[#C8E8F5] rounded-[18px] p-4 shadow-sm">
            <div className="text-[10.5px] text-[#8ABAC8] uppercase tracking-wide font-mono mb-2">Kasbon</div>
            <div className="font-['Syne'] text-[20px] font-bold text-[#1A3A4A] leading-none mb-1">Rp 0</div>
            <div className="text-[11px] text-[#4A7A8A]">dari limit Rp 1jt</div>
            <div className="h-[4px] bg-[#F0FAFF] rounded-full mt-3 overflow-hidden">
              <div className="h-full bg-[#F5A940]" style={{ width: '0%' }}></div>
            </div>
          </div>
          <div className="bg-white border border-[#C8E8F5] rounded-[18px] p-4 shadow-sm">
            <div className="text-[10.5px] text-[#8ABAC8] uppercase tracking-wide font-mono mb-2">Estimasi Gaji</div>
            <div className="font-['Syne'] text-[20px] font-bold text-[#1A3A4A] leading-none mb-1">Rp 4.2jt</div>
            <div className="text-[11px] text-[#4A7A8A]">bersih bulan ini</div>
            <div className="h-[4px] bg-[#F0FAFF] rounded-full mt-3 overflow-hidden">
              <div className="h-full bg-[#3AAD7A]" style={{ width: '100%' }}></div>
            </div>
          </div>
        </div>
